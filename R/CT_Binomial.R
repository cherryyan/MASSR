...
#' @param ushape The shape of the efficacy boundaries (default is "obf"). Eithera function specifying the shape or one of "pocock", "obf" (the default), "triangular" and "fixed".
...
#' @param x0_1 First stage event rate of control group. It has to be a one column matrix with the same number of rows as x1.
...
#' @examples
#' CT_Binomial(combF = "Fisher",a = 3, sel_rule = 2, n1=NULL, N2=NULL, initialSS = 2, r21 = 2, rmax= 1.5, alpha = 0.025, ushape = "obf" ,lfix=0, pi0=0.2, piT=0.4, x0_1=matrix(0.21,nrow=2,ncol=1), x1=matrix(c(0.19,0.20,0.35,0.49,0.37,0.30),nrow=2,ncol=3), beta=0.2,ConditionalPower = "planned",ssr = 1,delta = NULL)
#' CT_Binomial(combF = "Inverse Normal",a = 3, sel_rule = 1, n1=NULL, N2=NULL, initialSS = 2, r21 = 2, rmax= 1.5, alpha = 0.025, ushape = "obf" ,lfix=0, pi0=0.2, piT=0.4, x0_1=matrix(0.21,nrow=1,ncol=1), x1=matrix(c(0.35,0.30,0.22),nrow=1), beta=0.2,ConditionalPower = "observed",ssr = 1 ,delta = NULL)

#' @export


CT_Binomial = function(combF, a, sel_rule, n1=NULL, N2=NULL, initialSS = 2, r21, rmax, alpha = 0.025, ushape = "obf" ,lfix=0, pi0, piT, x0_1, x1, beta, ConditionalPower, ssr, delta = NULL,side = "upper"){
  ...
  macro_ct_ce_mcp = function(combF, Calpha, alpha1, alpha0 = alpha0,z1 = z1){
  ...
  J = 2
  matrix1 = matrix(1,nrow=1,ncol=a)
  alpha0 = 1-pnorm(lfix)
  theta0 = abs(piT - pi0)
  ...
  if (ConditionalPower == "planned"){
    ...
    n2_new_combF = function(ssr=ssr,combF = combF){
      ...
      B = Bf(p1=matrix(p1ih_allT,nrow=wh,ncol=1),combF=combF)
      Otheta = matrix_summary( (x1-(x0_1 %*% matrix1))*sel_tr,1,"max")
      if (side == "lower"){
        Otheta = matrix_summary( (-x1+(x0_1 %*% matrix1))*sel_tr,1,"max")
      }
      Opi_bar = (x1+(x0_1 %*% matrix1))/2
            ...
            if (cp_comb[i,]>=1-beta){
              ...
              if (sum_sel_tr[i,] == 1){
                ...
                if (cpdiff_n2max<=0){
                  ...
                  cp_new[i,] = round(f4(x=n2_new[i,],k1=k1,bi=bi,theta=Otheta[i,],sigma=sigma,cov1=cov1),2)+ 1 - beta
                }
              }
            }
          }
